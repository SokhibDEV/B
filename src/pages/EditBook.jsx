...
const EditBook = () => {
  ...
  const hadleEditBook = () => {
    ...
    setLoading(true);
    axios
      .put(`https://bookstore-i3kd.onrender.com/books/${id}`, data)
      .then(() => {
        setLoading(false);
        enqueueSnackbar("Book Edited succsessfully!", { variant: "success" });
        navigate("/");
      })
      .catch((error) => {
        setLoading(false);
  ...
  return (
    <div className='p-4'>
      ...
      <div className=' flex flex-col border-2 border-sky-400 rounded-xl w-[600px] p-4 mx-auto'>
        ...
        <button className='m-8 p-2 bg-sky-300' onClick={hadleEditBook}>
          Save
        </button>
      </div>
    </div>
